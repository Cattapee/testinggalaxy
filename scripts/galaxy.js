...
export class Galaxy {
  constructor(scene, arrayData = null) {
    ...
    this.stars = this.arrayData
      ? this.generateStarsFromArray()
      : this.generateStars();
    this.haze = this.arrayData
      ? this.generateHazeFromArray()
      : this.generateHaze();
    this.nebulae = this.arrayData
      ? this.generateNebulaeFromArray()
  ...
  determineRegion(pos) {
    ...
    if (dist < config.OUTER_CORE_X_DIST || dist < config.OUTER_CORE_Y_DIST)
      return "core";
    if (
      Math.abs(pos.x) < config.BAR_LENGTH &&
      Math.abs(pos.y) < config.BAR_WIDTH
    )
      return "bar";
